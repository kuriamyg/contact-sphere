...
| -------------------------------- | ------------ | ---------------------------- |
| `contact-management-api-staging` | `staging`    | the Vercel preview origin(s) |
| `contact-management-api`         | `production` | the Vercel production origin |
...
- Environment variable `API_URL` (server-side only, not `NEXT_PUBLIC_`):
  - Production → `https://contact-management-api.onrender.com`
  - Preview → `https://contact-management-api-staging.onrender.com`
